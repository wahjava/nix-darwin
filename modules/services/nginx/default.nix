...

{
  ...
  config = mkIf cfg.enable {
    ...
    in [
      ...
      { assertion = elem "nginx" config.users.knownGroups; message = "set users.knownGroups to enable nginx group"; }
      { assertion = elem "nginx" config.users.knownUsers; message = "set users.knownUsers to enable nginx user"; }
    ...
    launchd.daemons.nginx = {
      ...
      script =
        ...
        ${cfg.package}/bin/nginx -c ${configPath} -p ${cfg.stateDir}
        '';
    ...
    users.users.nginx = {
      ...
      gid = lib.mkDefault config.users.groups.nginx.gid;
    };

    users.groups.nginx = {
      name = "nginx";
      description = "Nginx user group";
